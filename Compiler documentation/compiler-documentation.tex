...
The type checker traverse through the output from the context analyser (the AST with symbol tables) 
to check all the types and their values. 
The output of the type checker is the AST with the symbol tables with possibly refined type annotations.

%TODO
The type checker validates if 

The type checker checks if the type and the value correspond to each other. For example it is

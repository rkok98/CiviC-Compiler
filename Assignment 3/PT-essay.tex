...
\par The DFA diagram (see Figure ~\ref{fig:DFA}) has been minimized using Hopcroft's algorithm and resulted in the following diagram:
\begin{figure}[h]
    \centering
\begin{tikzpicture}[shorten >=1pt,node distance=2cm,on grid,auto] 
    \node[state, accepting, initial](0) {0};
...
\par Following direct-coded scanner has been constructed based on on the minimized DFA diagram (see Figure ~\ref{fig:Min-DFA}):
